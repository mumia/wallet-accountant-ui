import Styled, { ExecutionContext } from "styled-components";
import { getThemeModeColors } from "../config/theme/themeVariables";

const Main = Styled.div`
  padding: 0px 30px 20px;
  min-height: 715px;
  background-color: transparent;
  &.grid-boxed{
    padding: 0px 180px 20px;
    @media only screen and (max-width: 1599px){
      padding: 0px 130px 20px;
    }
    @media only screen and (max-width: 1399px){
      padding: 0px 50px 20px;
    }
    @media only screen and (max-width: 991px){
      padding: 0px 30px 20px;
    }
  }

  .doughnutchart-inner{
    position: relative;
    .doughnutchart-inner-text{
      position: absolute;
      top: 50%;
      left: 50%;
      text-align: center;
      width: 200px;
      line-height: 1;
      margin-bottom: 0px;
      display: inline-block;
      transform: translate(-50%, -50%);
      .doughnutchart-inner-content{
        font-size: 30px;
        font-weight: 600;
        line-height: 1;
        color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).darkText};
        display: block;
      }
      .doughnutchart-inner-label{
        font-size: 15px;
        color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText};
      }
    }
    canvas{
      margin: 0 auto;
    }
  }
  .ant-switch{
    .anticon svg{
      color: #fff;
    }
  }
  /* ant breadcrumb *
  .ant-breadcrumb {
    a{
      color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText};
    }
  }
  
  .page-header-actions{
    .ant-btn-white{
      color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText} !important;
      background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).mainBackgroundLight};
      &:hover{
        background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).mainBackgroundLight} !important;
      }
    }
  }
  /* Card Title *
  .ant-card{
    h1,
    h2,
    h3,
    h4,
    h5,
    h6{
      color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).darkText};
    }
  }
  .ant-card{
    &.ninjadash-ghost-card{
      .ant-card-body{
        background: linear-gradient(90deg, rgb(95, 99, 242), rgb(255, 105, 165));
      }
    }
  }
  
  .ant-card-head-title{
    color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).darkText};
  }
  .ant-card-bordered{
    background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).whiteBackground};
    border-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorDefault};
  }
  .ant-card-head{
    border-bottom-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorDefault};
    background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).whiteBackground};
    .ninjadash-card-title-wrap{
      display: flex;
      justify-content: space-between;
      align-items: center;
      .ninjadash-card-title-text{
        font-size: 18px;
        font-weight: 600;
        display: inline-block;
        margin-left: 0;
        color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).darkText};
        .ninjadash-card-subtitile{
          font-size: 12px;
          margin-left: 6px;
          color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText};
        }
      }
      .ninjadash-card-title-extra-text{
        display: flex;
        align-items: center;
        position: relative;
        top: 3px;
        .ninjadash-total-chart-total{
          font-size: 18px;
          font-weight: 600;
          color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).darkText};
          @media only screen and (max-width: 991px){
            font-size: 15px;
          }
        }
        .ninjadash-total-chart-status{
          display: flex;
          align-items: center;
          font-size: 14px;
          font-weight: 500;
          margin-left: 8px;
          @media only screen and (max-width: 991px){
            font-size: 12px;
          }
          i,
          svg{
            width: 22px;
            height: 22px;
            margin-right: -1px;
            margin-right: -1px;
            @media only screen and (max-width: 991px){
              width: 18px;
              height: 18px;
            }
          }
          &.ninjadash-total-chart-status-growth{
            color: ${(props: ExecutionContext) => props.theme.colors.successColor};
          }
          &.ninjadash-total-chart-status-down{
            color: ${(props: ExecutionContext) => props.theme.colors.dangerColor};
          }
        }
      }
    }
  }

  .ant-card-body{
    p{
      color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText};
    }
  }

  .ninjadash-card-nav{
    ul{
      display: flex;
      align-items: center;
      margin: 0 -10px 2px 0;
      margin: 0 -10px 2px 0;
      li{
        margin: 5px 0;
        a{
          font-size: 13px;
          font-weight: 500;
          display: inline-flex;
          align-items: center;
          border-radius: 6px;
          min-height: 30px;
          padding: 0 12px;
          transition: .3s;
          color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayLightText};
          &:hover{
            color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).menuActive};
          }
        }
        &.ninjadash-active{
          a{
            background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).menuActiveBg};
            color: ${(props: ExecutionContext) => props.theme.colors.primaryColor};
          }
        }
      }
    }
  }

  .ant-card-rtl .ant-card-extra{
    margin-right: 0 !important;      
  }
  .ant-tabs-tab span svg {        
    padding-left: 5px;
  }
  .ant-btn{
    &.ant-btn-light{
      color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).whiteText};
      &.btn-transparent,
      &.btn-outlined{
        color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText};
      }
      &.btn-outlined:hover{
        color: ${(props: ExecutionContext) => props.theme.colors.primaryColor};
      }
    }
  }
  /* Pagination Style *
  .ant-pagination-item{
    border-radius: 4px;
  }
  .ant-pagination-item a{
    font-weight: 400;
  }
  .ant-pagination-next svg{
    fill: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText};
  }

  .ant-pagination-options .ant-select-selector{
    padding: 0 6.5px
  }
  .ant-pagination-options .ant-select .ant-select-arrow{
    svg{
      width: 10px;
      fill: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText};
    }
  }
  
  /* Picker Under Input *
  .ant-form-item-control-input .ant-picker {
    padding: 0 0 0 12px !important;
  }

  /* progress bars *

  .ant-progress {
    display: inline-flex !important;
    align-items: center;
    .ant-progress-inner{
      /* background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).mainBackgroundLight}; */
      background-color: transparent;
    }
    .ant-progress-text{
      color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText};
    }
    .ant-progress-circle-trail{
      background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).mainBackgroundLight};
    }
  }

  .ant-progress>div {
    display: flex;
    flex-direction: column;
  }

  .ant-progress .ant-progress-outer {
    margin-right: 0 !important;
    padding-right: 0 !important;
  }

  .ant-progress .ant-progress-text {
    order: 0;
    align-self: flex-end;
    text-align: center;
  }

  .ant-progress-status-warning .ant-progress-bg {
    background: #fa8b0c;
  }

  /* progress bars *
  
  @media only screen and (max-width: 1199px){
    padding: 0px 15px;
  }
  @media only screen and (max-width: 991px){
    min-height: 580px;
  }
  .w-100{
    width: 100%;
  }
  .product-sidebar-col{
    @media only screen and (max-width: 767px){
      order: 2;
    }
  }
  .ant-skeleton-paragraph{
    margin-bottom: 0;
  }

  /* Button Group *
  .ant-btn-group{
    .ant-btn-default{
      border-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorSecondary};
      background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).mainBackgroundLight}; 
    }
  }

  /* ant skeleton *
  .ant-skeleton{
    &.ant-skeleton-active{
      .ant-skeleton-content {
        .ant-skeleton-title{
          background: linear-gradient(90deg, 
          ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorDefault} 25%, 
          ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorSecondary} 37%, 
          ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorDefault} 63%);
          background-size: 400% 100%;
        }
        .ant-skeleton-paragraph{
          >li{
            background: linear-gradient(90deg, 
            ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorDefault} 25%,
            ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorSecondary} 37%,
            ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorDefault} 63%);
            background-size: 400% 100%;
          }
        }
      }
      .ant-skeleton-avatar{
        background: linear-gradient(90deg, 
        ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorDefault} 25%, 
        ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorSecondary} 37%, 
        ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorDefault} 63%);
        background-size: 400% 100%;
      }
    }
    .ant-skeleton-content {
      .ant-skeleton-title{
        background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorDefault};
      }
      .ant-skeleton-paragraph{
        >li{
          background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorDefault};
        }
      }
    }
    .ant-skeleton-header{
      .ant-skeleton-avatar{
        background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorDefault};
      }
    }
  }

  /* ant picker *
  .ant-picker{
    background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).inputBg};
    border-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorSecondary};
    input{
      color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText};
      &::placeholder{
          color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).lightText};
      }
    }
    .ant-picker-suffix{
      svg{
          color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText};
      }
    }
  }

  /* ant checkbox *
  .ant-checkbox{
    &:hover{
      .ant-checkbox-inner{
        border-radius: 4px;
      }
    }
    &.ant-checkbox-checked{
      &:after{
        border-color: ${(props: ExecutionContext) => props.theme.colors.primaryColor};
        border-radius: 4px;
      }
      .ant-checkbox-inner{
        background-color: ${(props: ExecutionContext) => props.theme.colors.primaryColor};
        border-color: ${(props: ExecutionContext) => props.theme.colors.primaryColor};
        &:after{
          top: 44%;
          border-color: #fff;
        }
      }
    }
    &.ant-checkbox-indeterminate{
      .ant-checkbox-inner{
        &:after{
          background-color: ${(props: ExecutionContext) => props.theme.colors.primaryColor};
        }
      }
    }
  }
  .ant-checkbox-wrapper{
    &.ant-checkbox-wrapper-disabled{
      opacity: .5;
    }
    .ant-checkbox{
      &+span{
        color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText};
      }
    }
  }
  .ant-checkbox-wrapper,
  .ant-checkbox{
    &:hover{
      .ant-checkbox-inner{
        border-color: ${(props: ExecutionContext) => props.theme.colors.primaryColor};
      }
    }
    .ant-checkbox-input{
      &:focus{
        &+.ant-checkbox-inner{
          border-color: ${(props: ExecutionContext) => props.theme.colors.primaryColor};
        }
      }
    }
  }
  .ant-checkbox-inner{
    border-radius: 4px;
    background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).whiteBackground};
    border-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorCheckbox};
  }

  /* ant alert *
  .ant-alert-closable{
    .ant-alert-message{
      margin-left: 15px;
    }
  }

  .ant-alert-with-description .ant-alert-description{
    display: inline-block;
  }

  /* ant Calendar Picker *
  .ant-picker-calendar{
    .ant-badge-status-text{
      color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText};
    }
  }
  .ant-picker-calendar-header .ant-picker-calendar-year-select{
    @media only screen and (max-width: 400px){
      width: 50% !important;
    }
  }
  .ant-picker-calendar-header .ant-picker-calendar-month-select{
    @media only screen and (max-width: 400px){
      width: calc(50% - 8px) !important
    }
  }

  /* Card Grid *
  .card-grid-wrap{
    .ant-card-grid{
      @media only screen and (max-width: 575px){
        width: 50% !important
      }
    }
  }

  /* Drawer *
  .atbd-drawer{
    .ant-card-body{
      text-align: center;
      .ant-drawer-wrapper-body {
        text-align: left;
      }
    }
  }
  .drawer-placement{
    @media only screen and (max-width: 400px){
      text-align: center;
    }
    .ant-radio-group{
      @media only screen and (max-width: 400px){
        margin-bottom: 15px;
      }
    }
  }
  .ant-drawer-content-wrapper{
    @media only screen and (max-width: 400px){
      width: 260px !important;
    }
    @media only screen and (max-width: 375px){
      width: 220px !important;
    }
  }

  /* Input *
  .input-wrap{
    @media only screen and (max-width: 991px){
      min-height: 500px;
    }
    input::placeholder{
      color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).lightText};
    }
  }
  /* Modal Buttons *
  .modal-btns-wrap{
    margin: 0 -5px;
  }
  /* spinner *
  .ant-spin{
    margin-left: 20px;
    &:last-child{
      margin-left: 0;
    }
  }

  /* Column Cards Wrapper *
  .columnCardsWrapper{
    background: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).darkBackground};
    border-radius: 4px;
    padding: 50px 30px 25px;
  }
  .columnCardsWrapper .ant-card{
    background: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).whiteBackground};
  }
  .columnCardsWrapper .ant-card-head{
    background: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).whiteBackground};
  }
  .ant-card-grid{
    color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText};
    box-shadow: 1px 0 0 0 ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorDefault}, 0 1px 0 0 
    ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorDefault}, 1px 1px 0 0 
    ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorDefault}, 1px 0 0 0 
    ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorDefault} inset, 0 1px 0 0 
    ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorDefault} inset     
  }

  /* Ant Collapse *
  .ant-collapse{
    background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).whiteBackground};
    border-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorDefault};
    border-radius: 6px;
    > .ant-collapse-item{
      &:last-child{
        border-radius: 0 0 6px 6px;
        background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).lightBackground};
        >.ant-collapse-header{
          border-radius: 0 0 6px 6px;
        }
      }
      .ant-collapse-item{
        &:last-child{
          background-color: transparent;
        }
        .ant-collapse-header{
          border-radius: 6px;
        }
      }
      .ant-collapse-content{
        background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).whiteBackground};
        .ant-collapse-content-box{
          background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).whiteBackground};
        }
      }
    }
    .ant-collapse-header{
      .ant-collapse-header-text{
        color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).darkText};
      }
    }
  }
  .ant-collapse.ant-collapse-icon-position-left .ant-collapse-header{
    color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText};
    padding: 12px 16px 10px 45px;
    background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).lightBackground};
    @media only screen and (max-width: 1299px){
      padding: 12px 16px 10px 30px;
    }
  }
  .ant-collapse-content p{
    color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayLightText};
    margin-bottom: 0;
  }
  .ant-collapse-content > .ant-collapse-content-box {
    padding: 20px 20px 20px;
  }
  .ant-collapse-content > .ant-collapse-content-box .ant-collapse-content-box{
    padding: 10.5px 20px;
  }
  .ant-collapse.ant-collapse-borderless{
    background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).whiteBackground};
    > .ant-collapse-item{
      &:last-child{
        border-radius: 0px;
      }
    }
  }
  .ant-collapse > .ant-collapse-item,
  .ant-collapse .ant-collapse-content{
    border-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorDefault};
  }
  .ant-collapse > .ant-collapse-item.ant-collapse-item-disabled .ant-collapse-header{
    color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).extraLightText} !important;
  }

  .ant-collapse > .ant-collapse-item .ant-collapse-header .ant-collapse-arrow{
    font-size: 8px;
    position: relative;
    top: -3px;
    right: 0;
  }

  .ant-collapse .ant-collapse {
    border: 0 none;
    background: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).whiteBackground};
  }

  .ant-collapse .ant-collapse > .ant-collapse-item {
    border-bottom: 0;
  }
  .ant-collapse .ant-collapse .ant-collapse-header{
    border-radius: 6px 6px 0 0;
    border: 1px solid ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorDefault};
    background: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).lightBackground};
  }
  .ant-collapse .ant-collapse .ant-collapse-content{
    margin: 20px 0 0 0;
    border: 1px solid ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorDefault};
    border-radius: 0;
  }

  /* Ant Radio *
  .ant-radio-wrapper{
    .ant-radio{
      &+span{
        color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText};
      }
    }
  }
  .ant-radio-button-wrapper-checked:not(.ant-radio-button-wrapper-disabled):focus-within{
    box-shadow: 0 0;
  }
  .ant-radio-button-wrapper-checked:not(.ant-radio-button-wrapper-disabled){
    background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).menuActive};
  }
  .ant-radio-button-wrapper-checked:not([class*=' ant-radio-button-wrapper-disabled).ant-radio-button-wrapper:first-child{
    border-right-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).menuActive};
  }
  .ant-radio-button-wrapper-checked:not(.ant-radio-button-wrapper-disabled):first-child{
    border-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).menuActive};
  }
  .ant-radio-button-wrapper{
    height: 48px;
    line-height: 46px;
    padding: 0 25.25px;
    color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText};
    background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).inputBg};
    border-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorDefault};
    @media only screen and (max-width: 1024px){
      padding: 0 10px;
    }
    @media only screen and (max-width: 379px){
      height: 40px !important;
      line-height: 38px !important;
      font-size: 12px;
      padding: 0 6px;
    }
    &:not(:first-child){
      &:before{
        background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorDefault};
      }
    }
    &.ant-radio-button-wrapper-checked{
      color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).whiteText};
    }
  }

  /* Select *
  .ant-tree-select .ant-select-selector{
    height: 42px;
  }
  .ant-select:not(.ant-select-customize-input) {
    .ant-select-selector{
      background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).inputBg};
      border-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorSecondary};
    }
  }
  .tag-select-list{
    margin-bottom: -10px;
    .ant-select{
      margin-bottom: 10px;
    }
  }
  .ant-select-selector{
    border-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorSecondary} !important;
  }

  .ant-select{
    .ant-select-selector {
      .ant-select-selection-item{
        color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText};
      }
    }
    &.ant-select-disabled{
      opacity: .6;
    }
    &.ant-select-multiple{
      .ant-select-selection-item{
        height: 24px;
        align-items: center;
        color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText};
        padding-left: 8px;
        background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).whiteBackground};
        border-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorSecondary};
      }
    }
    
    &.ant-select-lg{
      height: 50px;
      line-height: 48px;
      .ant-select-selector{
        height: 50px !important;
        line-height: 48px;
      }
      .ant-select-selection-item{
        line-height: 48px !important;
        padding-left: 8px;
      }
      &.ant-select-multiple.ant-select-lg .ant-select-selection-item{
        height: 32px;
        line-height: 32px !important;
      }
    }
    &.ant-select-multiple.ant-select-sm{
      .ant-select-selection-item{
        height: 16px;
        line-height: 14px;
        font-size: 11px;
      }
    }
    .ant-select-arrow{
      svg{
        color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText};
      }
    }
  }

  /* Slider *
  .slider-with-input{
    .ant-slider{
      margin-left: 15px;
    }
    .slider-with-input__single{
      margin-bottom: 15px;
    }
    .ant-input-number{
      background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).inputBg};
      border-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorDefault};
    }
    .ant-input-number-input{
      background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).inputBg};
      border-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorDefault};
    }
  }

  .ant-slider-mark-text{
    color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText};
  }

  .anticon svg{
    color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayLightText};
  }

  .ant-slider-handle{
    background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).inputBg}
  }

  /* ant input *
  .ant-input,
  .ant-input-affix-wrapper{
    color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText};
    background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).inputBg};
    border-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorSecondary};
    &:focus{
      border-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorSecondary};
      box-shadow: 0 0;
    }
    input.ant-input{
      &:focus{
        border-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorSecondary};
        box-shadow: 0 0 !important;
      }
    }
  }

  .ant-input-affix-wrapper{
    &:not(.ant-input-affix-wrapper-disabled){
      &:hover{
        border-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorSecondary};
      }
    }
  }
  .ant-input-affix-wrapper-focused{
    box-shadow: 0 0;
  }

  /* Taglist *
  .ant-tag{
    margin: 5px;
  }
  .taglist-wrap{
    margin: -5px;
    .ant-tag {
      line-height: 22px;
      padding: 0 10.2px;
      border: 0 none;
      margin: 5px;
      color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText};
      &.ant-tag-has-color{
        color: #fff !important;
      }
      &.ant-tag-magenta{
        color: #eb2f96;
      }
      &.ant-tag-red{
        color: #f5222d;
      }
      &.ant-tag-volcano{
        color: #fa541c;
      }
      &.ant-tag-orange{
        color: #fa8c16;
      }
      &.ant-tag-gold{
        color: #faad14;
      }
      &.ant-tag-line{
        color: #a0d911;
      }
      &.ant-tag-green{
        color: #a0d911;
      }
      &.ant-tag-cyan{
        color: #13c2c2;
      }
      &.ant-tag-blue{
        color: #1890ff;
      }
      &.ant-tag-geekbule{
        color: #2f54eb;
      }
      &.ant-tag-purple{
        color: #722ed1;
      }
    }
  }

  .ant-tag {
    color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).darkText};
    background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).darkBackground};
    border-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorSecondary};
    a{
      color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).darkText};
    }
    svg{
      color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText};
    }
  }

  /* ant empty *
  .ant-empty-description{
    color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText};
  }

  /* Steps style *
  .ant-steps-item-finish,
  .ant-steps-item-wait {
    .ant-steps-item-icon{
      background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).whiteBackground};
    }
    > .ant-steps-item-container {
      > .ant-steps-item-content {
        > .ant-steps-item-title{
          color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText};
        }
      }
    }
  }

  .steps-content{
    color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText};
  }

  .ant-steps-item-wait {
    .ant-steps-item-icon{
      border-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorDefault};
      .ant-steps-icon{
        color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText};
      }
    }
  }

  .ant-steps-item-process{
    > .ant-steps-item-container {
      > .ant-steps-item-content {
        > .ant-steps-item-title{
          color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).darkText};
        }
      }
    }
  }

  /* Timepicker List *
  .timepicker-list{
    margin: -5px;
    .ant-picker{
      margin: 5px;
    }
  }

  /* ant statistics *
  .ant-statistic {
    .ant-statistic-title{
      color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayLightText};
    }
    .ant-statistic-content{
      color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).darkText};
    }
  }
  .ant-statistic-content{
    .anticon {
      &.anticon-arrow-up{
        svg{
          color: ${(props: ExecutionContext) => props.theme.colors.successColor};
        }
      }
      &.anticon-arrow-down{
        svg{
          color: ${(props: ExecutionContext) => props.theme.colors.dangerColor};
        }
      }
    }
  }

  /* Ant Menu *
  .ant-menu{
    background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).whiteBackground};
    border-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorDefault};
    .ant-menu-submenu-title{
      svg,
      .ant-menu-title-content,
      .ant-menu-submenu-arrow{
        color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayLightText};
      }
      &:active{
        background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).menuActiveBg};
      }
    }
    .ant-menu-item{
      &:active{
        background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).menuActiveBg};
      }
      &.ant-menu-item-selected{
        &:after{
          border-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).menuActive};
        }
        .ant-menu-item-icon{
          svg{
            color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).menuActive};
          }
        }
        .ant-menu-title-content{
          color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).menuActive};
        }
      }
      &.ant-menu-item-disabled{
        .ant-menu-item-icon{
          color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).extraLightText};
        }
        .ant-menu-title-content{
          opacity: .5;
          color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).extraLightText};
        }
      }
      &.ant-menu-item-active{
        color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).menuActive};
      }
    }
    .ant-menu-submenu {
      &.ant-menu-submenu-selected{
        .ant-menu-submenu-title{
          span{
            color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).menuActive};
          }
        }
      }
    }
    &.ant-menu-inline{
      .ant-menu-submenu-title svg{
        position: relative;
        top: 2px;
      }
      .ant-menu-submenu {
        &.ant-menu-submenu-open,
        &.ant-menu-submenu-active{
          .anticon {
            svg{
              color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).menuActive};
            }
          }
          svg{
            color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).menuActive};
          }
          .ant-menu-submenu-arrow{
            color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).menuActive};
          }
        }
      }
    }
    &:not(.ant-menu-horizontal){
        .ant-menu-item-selected,
        .ant-menu-submenu-open{
            background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).menuActiveBg};
            &.ant-menu-item-active{
                color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).menuActive};
            }
        }
    }
  }

  .ant-menu-horizontal{
    &:not(.ant-menu-dark){
      > .ant-menu-submenu{
        &:hover{
          .ant-menu-submenu-title{
            svg,
            .ant-menu-title-content{
              color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).menuActive};
            }
          }
          &:after{
            border-bottom-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).menuActive};
          }
        }
      }
    }
  }

  /* ant result *
  .ant-result{
    &.ant-result-success{
      .anticon svg{
        color: ${(props: ExecutionContext) => props.theme.colors.successColor};
      }
    }
    &.ant-result-info{
      .anticon svg{
        color: ${(props: ExecutionContext) => props.theme.colors.infoColor};
      }
    }
    &.ant-result-warning{
      .anticon svg{
        color: ${(props: ExecutionContext) => props.theme.colors.warningColor};
      }
    }
    &.ant-result-error{
      .anticon svg{
        color: ${(props: ExecutionContext) => props.theme.colors.dangerColor};
      }
    }
    .ant-result-title{
      color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).darkText};
    }
    .ant-result-subtitle{
      font-size: 13px;
      color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText};
    }
    .ant-result-content{
      background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).mainBackgroundLight};
      .ant-typography{
        color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText};
        a{
          margin-left: 4px;
        }
      }
    }
  }

  .ant-result-extra{
    .ant-btn-default{
      color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText};
      border-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorSecondary};
      background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).mainBackgroundLight};
    }
  }

  /* Ant Comment *
  .ant-comment-inner{
    padding: 0;
    .ant-comment-content-author,
    .ant-comment-content-author-name{
      >*{
        color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText};
      }
    }
  }
  .ant-comment-content-detail p{
    color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayLightText};
  }
  .ant-list-items{
    padding-top: 22px;
  }
  .ant-list-items li:not(:last-child){
    margin-bottom: 22px;
  }
  .ant-comment:not(:last-child){
    margin-bottom: 22px;
  }
  .ant-comment-nested{
    margin-top: 22px;
  }
  .ant-comment-actions li span{
    color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).extraLightText};
  }
  .ant-comment-content-detail textarea{
    resize: none;
    min-height: 170px;
    border-radius: 5px;
    background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).inputBg};
    border-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorSecondary};
  }
  .nested-comment-wrapper  {
    .comment-title{
      font-size: 12px;
      padding-bottom: 10px;
      margin-bottom: 22px;
      border-bottom: 1px solid ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorSecondary};
    }
  }

  .ant-list-split .ant-list-header{
    color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText};
    border-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorDefault};
  }

  /* ant list *
  .ant-list{
    &.ant-list-bordered{
      color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText};
      border-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorSecondary};
    }
    .ant-list-item-meta{
      padding: 10px 24px; 
      border-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorDefault};
    }
    .ant-list-item-meta-description{
      color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText};
    }
  }

  /* Vector Map *
  .rsm_map{
    min-height: 505px;
    .world-map{
      width: 100%;
      height: auto;
      @media only screen and (max-width: 1599px){
        height: 480px;
      }
      @media only screen and (max-width: 1399px){
        height: 400px;
      }
      @media only screen and (max-width: 575px){
        height: 400px;
      }
      @media only screen and (max-width: 767px){
        height: 300px;
      }
      @media only screen and (max-width: 575px){
        height: 250px;
      }
      @media only screen and (max-width: 479px){
        height: 350px;
      }
      @media only screen and (max-width: 375px){
        height: 240px;
      }
    }
    .controls{
      position: absolute;
      right: 30px;
      bottom: 30px;
      button{
        display: block;
        width: 27px;
        height: 27px;
        background: none;
        color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).whiteText};
        border: 1px solid ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorDefault};
        padding: 0;
        font-size: 15px;
        display: flex;
        align-items: center;
        justify-content: center;
        background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).whiteBackground};
        cursor: pointer;
        &:first-child{
          border-radius: 6px 6px 0 0;
        }
        &:last-child{
          border-radius: 0 0 6px 6px;
        }
        &:focus{
          outline: none;
        }
        svg{
          width: 10px;
          color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText};
        }
      }
      button + button{
        border-top: 0 none;
      }
    }
  }

  /* Star Active *
  a{
    i,
    span.fa{
      font-size: 16px;
      color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).extraLightText};
    }
    &.starDeactivate{
      i:before{
        content: "\f31b";
      }
    }
    &.starActive{
      i,
      span.fa{
        color: ${(props: ExecutionContext) => props.theme.colors.warningColor};
      }
      i:before,
      span.fa:before{
        color: ${(props: ExecutionContext) => props.theme.colors.warningColor};
        content: "\f005";
      }
    }
  }

  .ant-timeline{
    color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText};
    .ant-timeline-item-content{
      font-size: 16px;
    }
  }

  .ant-timeline-item-tail{
    border-width: 3px;
    border-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorDefault};
  }
  
  .ant-rate-content{
    font-weight: 500;
    color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText};
  }

  .ant-rate-text{
    color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText};
  }

  .ant-rate{
    .anticon {
      svg{
        color: #FA8B0C;
      }
    }
  }

  .ant-rate-star.ant-rate-star-zero span svg{
    color: #c6d0dc;
  }

  .account-card{
    .ant-card-head{
      .ant-card-extra{
        @media only screen and (max-width: 575px){
          padding-top: 0 !important;
        }
      }   
    }           
  }

  /* Rechart */
  .recharts-default-legend{
    .recharts-legend-item{
      min-width: 100px !important;
    }
  }

  /*  Radio */
  .ant-radio{
    &.ant-radio-disabled{
      opacity: .4;
      &+span{
        opacity: .4;
      }
    }
  }
  .radio-size-wrap{
    .ant-radio-button-wrapper{
      @media only screen and (max-width: 1450px){
        padding: 0 11.5px;
      }
    }
  }
  .ant-radio-wrapper:hover .ant-radio, 
  .ant-radio:hover .ant-radio-inner, 
  .ant-radio-input:focus + .ant-radio-inner{
    border-color: ${(props: ExecutionContext) => props.theme.colors.primaryColor};
    outline: none;
    box-shadow: 0 0;
  }

  .ant-radio-inner{
    transition: 0s;
    background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).inputBg};
    border-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorSecondary};
  }
  .ant-radio-checked{
    .ant-radio-inner{
      border-color: ${(props: ExecutionContext) => props.theme.colors.primaryColor};
    }
  }

  .ant-radio-button-wrapper{
    &.ant-radio-button-wrapper-disabled{
      opacity: .4;
    }
  }

  .ant-radio-button-wrapper{
    &.ant-radio-button-wrapper-checked{
      &.ant-radio-button-wrapper-disabled{
        background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).menuActive};
      }
    }
  }  

  /* Message  *
  
  .message-button-list{
    margin: -4px;
    .ant-btn {
      margin: 4px;
    }
  }
  
  /* Chart Label *
  
  .chart-label {
    display: flex;
    align-items: center;
    justify-content: center;
    margin-bottom: 6px;
    color: #5a5f7d;
  }

  .chart-label .label-dot {
    margin-left: 8px;
    width: 7px;
    height: 7px;
    border-radius: 50%;
  }

  .chart-label .label-dot.dot-success {
    background: #20c997;
  }

  .chart-label .label-dot.dot-info {
    background: #5f63f2;
  }

  .chart-label .label-dot.dot-warning {
    background: #fa8b0c;
  }

  .chart-label .label-dot {
    display: block;
    width: 7px;
    height: 7px;
    border-radius: 50%;
    margin-left: 10px;
  }

  // Ant comment action
  .ant-comment-actions{
    li{
      position: relative;
      &:not(:last-child){
        margin-right: 8px;
        padding-right: 8px;
        &:after{
          position: absolute;
          right: 0;
          top: 50%;
          transform: translateY(-50%);
          width: 1px;
          height: 12px;
          background-color: #C6D0DC;
          content: '';
        }
      }
      .com-time{
        cursor: default;
      }
      span{
        margin-right: 0;
      }
    }
  }

  .wizard-side-border{
    >.ant-card{
      .ant-card-body{
        padding: 0 25px !important;
      }
    }
    .checkout-successful{
      >.ant-card{
        .ant-card-body{
          padding: 25px !important;
          @media only screen and (max-width: 575px){
            padding: 15px !important;
          }
        }
      }
    }
    .payment-method-form.theme-light{
      .shipping-selection__card{
        .ant-card-body{
          padding: 25px 0 !important;
        }
      }
    }
    .shipping-selection__card{
      .ant-card-body{
        padding: 25px !important;
        @media only screen and (max-width: 575px){
          padding: 15px !important;
        }
      }
    }
    .atbd-review-order{
      .ant-card-body{
        padding: 25px 25px 0 !important;
        @media only screen and (max-width: 767px) {
          padding: 15px 15px 0 !important;
        }
      }
    }
    
    .ant-steps {
        padding: 50px;
        @media only screen and (max-width: 1399px) {
            padding: 25px;
        }
    }
    .steps-wrapper{
        padding: 50px;
        @media only screen and (max-width: 1399px) {
            padding: 25px;
        }
        border-right: 1px solid ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorDefault};
    }
  }
  .editor-compose > div {
    position: static;
    max-width: 100%;
    margin: 0 0 25px;
  }

  // Ant Dragger
  .ant-upload-drag{
    background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).whiteBackground} !important;
    border-radius: 10px !important;
    display: flex;
    align-items: center;
    min-height: 100px;
    border-color: #C6D0DC;
    &.ninjadash-uploader-large{
      min-height: 180px;
    }
    .ant-upload-drag-container{
      .ant-upload-text{
        margin-bottom: 0;
        font-size: 15px;
        color: #9299B8;
      }
    }
  }

  .ant-upload{
    .ant-btn-light{
      color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText};
      &:hover{
        border-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).menuActive};
      }
      svg{
        color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText};
      }
    }
  }

  .ant-upload-list-item{
    .ant-upload-list-item-info{
      border-radius: 6px;
      padding: 0px 8px;
      background-color: transparent;
      svg{
        color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText};
      }
      a{
        color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).menuActive};
      }
    }
  }

  .ant-table{
    background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).whiteBackground};
  }

  .ant-table-thead {
    > tr {
      > th{
        background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).mainBackgroundLight};
        border-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorDefault};
        color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).darkText};
      }
    }
  }
  .ant-table-tbody{
    >tr{
      >td{
        border-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorDefault};
        background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).whiteBackground};
        color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText};
      }
      &:hover{
        >td{
          background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).mainBackgroundLight};
        }
      }
    }
  }
  .ant-btn-transparent,
  .btn-outlined,
  .ant-btn-white
  {
    .anticon-loading{
      svg{
        color: ${(props: ExecutionContext) => props.theme.colors.primaryColor};
      }
    } 
  }
  .anticon-loading{
    svg{
      color: #fff;
    }
  }

  .ninjadash-action-count{
    padding-left: 8px;
  }
`;

const ErrorWrapper = Styled.div`
  display: flex;
  align-items: center;
  justify-content: center;
  flex-direction: column;
  width: 100%;
  min-height: 100vh;
  text-align: center;
  img{
    margin-bottom: 100px;
    max-width: 400px;
    width: 100%;
    @media only screen and (max-width: 575px){
      margin-bottom: 40px;
    }
  }
  .error-text{
    font-size: 60px;
    font-weight: 600;
    margin-bottom: 35px;
    color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).extraLightText};
  }
  p{
    font-size: 18px;
    font-weight: 500;
    margin-bottom: 26px;
    color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText};
  }
  button{
    height: 44px;
  }
`;

const BorderLessHeading = Styled.div`
  .ant-card{
    .ant-card-head{
      border: 0 none;
    }
    .ant-card-body{
      padding-top: 0 !important;
    }
  }
`;
const ChartPointHorizontal = Styled.div`
  .ninjadash-chartpoint{
    display: flex;
    align-items: center;
    justify-content: space-between;
    max-width: 330px;
    margin: 20px auto 0;
    .ninjadash-chartpoint__item{
      display: flex;
      align-items: center;
      .ninjadash-chartpoint__tika{
        display: block;
        width: 7px;
        height: 7px;
        margin-right: 6px;
        border-radius: 50%;
      }
      .ninjadash-chartpoint__label{
        font-size: 14px;
        color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayLightText};
      }
    }
  }
`;
const TableDefaultStyle = Styled.div`
  &.ninjadash-having-header-bg{
    .ant-table-content{
      table{
        tr{
          &:first-child{
            td{
              padding-top: 15px !important;
            }
          }
        }
      }
    }
  }
  .ant-table{
    background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).whiteBackground};
    .ant-table-cell{
      color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).darkText};
    }
  }
  .ant-table-content{
    .ant-table-thead{
      >tr{
        >th{
          &:not(:last-child):not(.ant-table-selection-column):not(.ant-table-row-expand-icon-cell):not([colspan]){
            &:before{
              display: none;
            }
          }
        }
        >th.ant-table-cell{
          text-transform: uppercase;
          font-size: 12px;
          font-weight: 500;
          color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayLightText};
          background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).statusBackground};
          border: 0 none;
          &:first-child{
            border-top-left-radius: 6px;
            border-bottom-left-radius: 6px;
            padding-left': 15px;
          }
          &:last-child{
            border-top-right-radius: 6px;
            border-bottom-right-radius: 6px;
            padding-right: 15px;
            text-align: right;
          }
        }
      }
    }
    .ant-table-tbody{
      >tr{
        .status{
          font-weight: 500;
          text-transform: capitalize;
          &.done{
            color: ${(props: ExecutionContext) => props.theme.colors.successColor};
            background: ${(props: ExecutionContext) => props.theme.colors.successColor}10;
          }
          &.pending{
            color: ${(props: ExecutionContext) => props.theme.colors.warningColor};
            background: ${(props: ExecutionContext) => props.theme.colors.warningColor}10;
          }
          &.blocked{
            color: ${(props: ExecutionContext) => props.theme.colors.dangerColor};
            background: ${(props: ExecutionContext) => props.theme.colors.dangerColor}10;
          }
        }
        &:hover{
          td.ant-table-cell{
            background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).whiteBackground} !important;
          }
        }
        &:first-child{
          >td.ant-table-cell{
            padding-top: 20px;
          }
        }
        >td.ant-table-cell{
          border: 0 none;
          font-size: 14px;
          &:first-child{
            font-size: 15px;
            padding-left: 0;
          }
          &:last-child{
            padding-right: 15px;
            text-align: right;
          }
        }
      }
    }
    tr{
      .ant-table-cell{
        padding: 10px 15px;
        .ninjadash-info-element{
          img{
            max-width: 32px;
            margin-right: 12px;
          }
      
          .ninjadash-info-element__text{
            font-size: 15px;
            font-weight: 500;
            text-transform: capitalize;
            color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).darkText};
          } 
        }
      }
    }
  }
  .ninjadash-recent-deals,
  .ninjadash-top-product{
    min-height: 385px;
  }

  .ninjadash-member-progress-table{
    .ant-progress{
      min-width: 120px;
    }
    .ant-table-content {
      .ant-table-tbody {
        >tr:first-child {
          >td.ant-table-cell{
            padding-top: 14px;
          }
        }
      }
    }
    .ant-progress-text{
      color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText};
    }
  }
  .ant-empty-description{
    color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayLightText};
  }
`;

const ButtonsGroupWrapper = Styled.div`
  margin-bottom: -25px;
  .button-group-single{
    margin-bottom: 15px;
    h4{
      font-size: 15px;
      margin-bottom: 8px;
    }
    .ant-btn-white{
        border: 1px solid ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorSecondary};
        background: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).whiteBackground} !important;
        // border-left-width: 0px;
        &:last-child{
          border-left-width: 1px;
        }
        &:hover{
          color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText} !important;
          background: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).generalBackground} !important;
        }
    }
    .ant-btn.ant-btn-light{
      border: 1px solid ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorSecondary};
      font-weight: 500;
      color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).extraLightText} !important;
      background: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).generalBackground} !important;
      &:last-child{
        border-left-width: 1px;
      }
    }
    
    .ant-btn-group{
      margin-bottom: 10px;
      &:not(last-child){
        margin-left: 20px;
      }
      button{
        font-size: 13px;
        span{
          line-height: 1;
        }
        svg{
          color: #fff;
        }
      }
    }
  }
`;

const BlockButtonsWrapper = Styled.div`
  .ant-btn-block{
    margin-bottom: 10px;
  }
`;

const ButtonSizeWrapper = Styled.div`
  display: flex;
  align-items: flex-end;
  flex-wrap: wrap;
`;

const BtnWithIcon = Styled.div`
  display: inline;
  .anticon i,
  .anticon svg,
  .anticon img{
    width: 10px;
    height: 10px;
  }
  .ant-btn-group button.active{
    border-left: 0px;
  }
`;

const AlertList = Styled.div`
  margin-top: -15px;
  .alert-empty-message{
    .ant-alert{
      padding: 14.5px 20px 14.5px!important;
      &.ant-alert-no-icon{
        padding: 14.5px 20px 14.5px!important;
      }
    }
    .ant-alert-icon{
      top: 15px !important;
    }
    .ant-alert-message{
      display: none !important;
    }
  }
`;

const AutoCompleteWrapper = Styled.div`
  .ant-select:not(:last-child){
    margin-bottom: 20px;
  }
  .auto-complete-input{
    .ant-select{
      width: 200px !important;
      @media only screen and (max-width: 575px){
        width: 100% !important;
      }
    }
  }
`;

const CalendarWrapper = Styled.div`
  .ant-picker-calendar{
    background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).whiteBackground};
    .ant-picker-panel{
      background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).whiteBackground};
      .ant-picker-calendar-date{
        border-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorDefault};
        .ant-picker-calendar-date-value{
          color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).lightText};
        }
        .ant-picker-calendar-date-content{
          .notes-month{
            span{
              color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText};
            }
          }
        }
      }
    }
  }
  .ant-picker-content{
    thead{
      tr{
        th,
        td{
          color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText};
        }
      }
    }
      tbody{
        tr{
          td{
            &:hover{
              .ant-picker-cell-inner{
                background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).mainBackgroundLight} !important;
              }
            }
            .ant-picker-calendar-date{
              .ant-picker-calendar-date-value{
                color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).lightText};
                opacity: .6;
              }
                &.ant-picker-calendar-date-today{
                  background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).inputBg};
                  &:hover{
                    .ant-picker-calendar-date-value{
                        color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).menuActive};
                    }
                  }
                }
            }
            &.ant-picker-cell-in-view{
              .ant-picker-calendar-date{
                .ant-picker-calendar-date-value{
                  opacity: 1;
                  color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).lightText};
                }
              }
            }
          }
        }
      }
  }
  .ant-select-single:not(.ant-select-customize-input) .ant-select-selector{
    height: 30px !important;
  }
  .ant-select-single .ant-select-selector .ant-select-selection-item,
  .ant-select-single .ant-select-selector .ant-select-selection-placeholder {
    line-height: 28px !important;
    color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText};
  }
  .ant-picker-calendar-full .ant-picker-panel .ant-picker-calendar-date-content{
    height: 105px;
    @media only screen and (max-width: 575px){
      height: 40px;
    }
  }
  .ant-radio-button-wrapper{
    height: 30px;
    line-height: 28px;
    @media only screen and (max-width: 575px){
      height: 30px !important;
      line-height: 28px !important;
    }
  }
`;

const DatePickerWrapper = Styled.div`
  .ant-picker{
    padding: 6px 11px 6px;
    width: 100%;
    border-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorSecondary};
    border-radius: 5px;
  }
  .ant-picker:not(:last-child){
    margin-bottom: 20px;
  }
  .ant-picker-input > input{
    color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText};
    &::placeholder{
      color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).lightText};
    }
  }
  .ant-picker-input > input[disabled]{
    opacity: .6;
  }
  .ant-picker-range .ant-picker-input > input{
    text-align: center;
  }
`;

const NotificationListWrapper = Styled.div`
  margin: -4px;
  button{
    margin: 4px;
    .anticon svg{
      color: #fff;
    }
  }
`;

const TagInput = Styled.div`
  padding: 12px;
  border-radius: 4px;
  border: 1px solid ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorSecondary};
  margin: -3px;
  .ant-tag{
    margin: 3px;
    font-size: 11px;
    padding: 0 4px;
    border: 0 none;
    height: 24px;
    display: inline-flex;
    align-items: center;
  }
`;

const PageHeaderWrapper = Styled.div`
  .ant-page-header{
    border: 1px solid ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorSecondary};
    border-radius: 5px;
  }
  .ant-page-header .ant-page-header-heading-left{
    margin: 2px 0;
  }
`;

const MessageStyleWrapper = Styled.div`
  .ant-btn-lg{
    font-size: 14px;
  }
`;

const BasicFormWrapper = Styled.div`
  .ant-form {
    .form-item{
      margin-bottom: 30px;
      label{
        font-weight: 500;
        display: block;
        margin-bottom: 15px;
        color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText};
      }
      .ant-cascader-picker{
        width: 100%;
        min-height: 48px;
        .ant-cascader-input{
          min-height: 48px;
        }
      }
    }
    .ant-input-affix-wrapper > input.ant-input{
      padding-top: 12px;
      padding-bottom: 12px;
    }
    .ant-input-affix-wrapper .ant-input-prefix svg{
      color: #9299B8;
    }
    .ant-form-item-label > label.ant-form-item-required:not(.ant-form-item-required-mark-optional)::before{
      position: relative;
      top: -2px;
    }
    input:-webkit-autofill,
    input:-webkit-autofill:hover, 
    input:-webkit-autofill:focus,
    textarea:-webkit-autofill,
    textarea:-webkit-autofill:hover,
    textarea:-webkit-autofill:focus,
    select:-webkit-autofill,
    select:-webkit-autofill:hover,
    select:-webkit-autofill:focus{
      background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).whiteBackground} !important;
    }
  }
  .ant-form-item-control-input{
    min-height: auto !important;
  }
  .ant-form-item,
  .ant-form-item-row{
    flex-flow: column;
    &:not(:last-child){
      margin-bottom: 26px;
    }
    &:last-child{
      margin-bottom: 0;
    }
    .ant-form-item-label{
      text-align: right;
      label{
        color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).darkText};
        height: fit-content;
        margin-bottom: 6px;
      }
    }
    .ant-form-item-control-input{
      input,
      textarea{
        color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText};
        background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).inputBg};
        border-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorSecondary};
        &:placeholder{
          color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).lightText};
        }
      }
      .ant-picker-input input{
        padding: 12px;
      }
      button{
        height: 44px;
      }
      .ant-input-affix-wrapper{
        padding: 0 20px;
      }
    }
    .ant-select {
      .ant-select-arrow{
        svg{
          color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText};
        }
      }
    }
    .ant-select-single{
      .ant-select-selector{
        padding: 0 20px;
        height: 50px !important;
        background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).inputBg};
        border-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorDefault} !important;
        .ant-select-selection-item{
          color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText};
          line-height: 46px !important;
          padding: 0 !important;
        }
        .ant-select-selection-placeholder{
          line-height: 46px !important;
        }
      }
    }
    .ant-radio-group {
      color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText};
    }
  }
  .setting-form-actions{
    margin: 48px 0 14px;
    @media only screen and (max-width: 575px){
      margin: 40px 0 14px;
    }
    button{
      border-radius: 6px;
      height: 44px;
      margin-bottom: 14px;
      &.ant-btn-light{
        border: 1px solid ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorDefault};
        background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).mainBackgroundLight};
        color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText};
      }
    }
  }
  .ant-form-item-control-input{
    .input-prepend{
      position: absolute;
      right: 0;
      display: inline-flex;
      align-items: center;
      justify-content: center;
      padding: 0 20px;
      height: 48px;
      border-radius: 0 4px 4px 0;
      z-index: 10;
      border: 1px solid ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorSecondary};
      background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).mainBackgroundLight};
      svg,
      i{
        color:${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText};
      }
      svg{
        width: 16px;
        height: 16px;
      }
    }
    .input-prepend-wrap{
      .ant-input-number{
        input{
          padding-left: 70px;
        }
      }
    }
    .ant-input-number{
      width: 100% !important;
      border: 1px solid ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorSecondary};
      background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).inputBg};
    }
  }
  .add-record-form{
    margin: 25px 0 35px 0;
    
    .record-form-actions{
      padding-right: 40px;
    }
    .ant-btn{
      height: 44px;
      font-size: 14px;
      font-weight: 500;
    }
    .ant-radio-group{
      margin-bottom: -4px;
      .ant-radio-wrapper{
        margin-bottom: 4px;
      }
    }
  }
  .adTodo-form{
    .btn-adTodo {
      font-size: 14px;
    }
  }

  .ninjadash-form-action{
    margin: -7.5px;
    button{
      font-size: 14px;
      font-weight: 500;
      border-radius: 6px;
      margin: 7.5px;
      padding: 6.4px 19px;
      &.ant-btn-light{
        height: 44px;
        color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText};
        background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorDefault};
        border-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorDefault};
      }
    }
    .ant-form-item{
      margin-bottom: 25px !important;
    }
    .ant-btn-light{
      background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).mainBackground};
    }
  }
  .ninjadash_color-picker{
    border: 1px solid ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorSecondary};
    border-radius: 4px;
    padding: 11px 14px;
    input{
      width: 100%;
      border: 0 none;
      background-color: #fff;
      &::-webkit-color-swatch{
        min-height: 18px;
        border: 1px solid #C6D0DC;
      }
    }
  }
  .ant-input-number{
    .ant-input-number-input{
      min-height: 46px;
    }
  }
`;

const CardToolbox = Styled.div`
  margin: 16px 0 10px;
  @media only screen and (max-width: 767px){
    margin: 8px 0 2px;
  }
  @media only screen and (max-width: 575px){
    margin: 2px 0 2px;
  }
  .ant-page-header.ninjadash-page-header-main{
    padding: 18px 30px 15px;
    @media only screen and (max-width: 767px){
      padding: 18px 15px 15px;
    }
  }
  .ant-page-header-heading{
    @media only screen and (max-width: 991px){
      flex-flow: column;
      align-items: center;
    }
    .ant-page-header-heading-title{
      color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).darkText};
    }
    .title-counter{
      color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText};
    }
    .ant-input-affix-wrapper{
      background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).inputBg};
      input{
        background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).inputBg};
      }
    }
  }

  .ant-page-header-heading-left{
    @media only screen and (max-width: 575px){
      flex-flow: column;
    }
    @media only screen and (max-width: 800px){
      max-width: 100%;
    }
    .ant-page-header-back{
      @media only screen and (max-width: 575px){
        margin: 0;
        padding: 0;
      }
    }
    .ant-page-header-heading-title{
      @media only screen and (max-width: 575px){
        margin: 0 0 8px;
        padding: 0;
      }
      &:after{
        @media only screen and (max-width: 575px){
          display: none;
        }
      }
    }
    .ant-page-header-heading-sub-title{
      @media only screen and (max-width: 575px){
        margin: 0;
        padding: 0;
        flex-flow: column;
      }
      .title-counter{
        @media only screen and (max-width: 575px){
          margin-bottom: 16px;
        }
      }
    }
  }

  .ant-page-header-heading-title{
    position: relative;
    // padding-left: 24px;
    // margin-left: 24px;
    @media only screen and (max-width: 767px){
      padding-left: 12px;
      margin-left: 12px !important;
    }
    &:after{
      position: absolute;
      left: 0px;
      top: 0;
      height: 100%;
      width: 1px;
      content: '';
      background: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorDefault};
    }
  }
  .ant-page-header-heading-sub-title{
    font-weight: 500;
    display: flex;
    align-items: center;
  }
  .ant-page-header-heading-extra{
    @media only screen and (max-width: 991px){
      margin-top: 10px;
    }
  }
  .ant-select{
    margin-left: 25px;
    @media only screen and (max-width: 575px){
      margin-left: 0;
    }
    .ant-select-selector{
      height: 46px !important;
      min-width: 350px;
      @media only screen and (max-width: 1299px){
        min-width: 250px;
      }
      @media only screen and (max-width: 1199px){
        min-width: 220px;
      }
      @media only screen and (max-width: 991px){
        min-width: 100%;
      }
    }
    .ant-select-selection-search-input{
      height: 46px;
      border-radius: 23px;
      border: 0 none;
      box-shadow: 0 5px 20px #9299B803;
      input{
        height: 46px !important;
        font-size: 14px;
      }
    }
  }
  .btn-add_new{
    display: flex;
    align-items: center;
    font-weight: 600;
    border-radius: 6px;
    height: 40px;
    padding: 0 14px;
    font-size: 14px;
    color: #fff;
    background-color: ${(props: ExecutionContext) => props.theme.colors.primaryColor};
    @media only screen and (max-width: 375px){
      padding: 0 10px;
    }
    svg{
      width: 14px;
      height: 14px;
      margin-left: 6px;
    }
    a{
      display: flex;
      align-items: center;
      svg{
        margin-left: 6px;
      }
    }
  }
  .ant-select-selection-search{
    .ant-input-suffix{
      svg{
        color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).lightText};
      }
    }
  }
`;

const FormGroupWrapper = Styled.div`
  .ant-card-body{
    @media only screen and (max-width: 767px){
      padding: 0 !important;
    }
  }
`;
const BannerCardStyleWrap = Styled.div`
  .ant-card-body{
    padding: 25px 25px 0 25px !important;
  }
`;

const FileCardWrapper = Styled.div`
  .file-list{
    min-height: 385px;
    .file-list__single{
      justify-content: space-between;
      align-items: center;
      &:not(:last-child){
        margin-bottom: 18px;
      }
      span{
        display: block;
        font-size: 12px;
        line-height: 1.42;
        &.file-name{
          font-size: 14px;
          font-weight: 500;
          color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).darkText};
        }
        &.file-size{
          margin: 2px 0;;
          color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText};
        }
        &.file-content-action{
          a{
            font-weight: 500;
            color: ${(props: ExecutionContext) => props.theme.colors.primaryColor};
          }
          a + a{
            margin-left: 8px;
          }
        }
      }
    }
    .file-single-info{
      width: 50%;
      align-items: center;
      .file-single-logo{
        margin-left: 16px;
        img{
          max-width: 42px;
        }
      }
    }
    .file-single-action{
      .ant-dropdown-trigger {
        color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).extraLightText};
      }
    }
  }
`;

const TableWrapper = Styled.div`
  .ant-pagination-prev, .ant-pagination-next {
    line-height: 28px !important;
    transform: rotateY(180deg)
  }
  .ant-table table{
    text-align: left;
  }
  .ant-table-thead > tr > th{
    text-align: left;
  }
  span.anticon.anticon-right{
    transform: rotateY(180deg)
  }
  span.anticon.anticon-left{
    transform: rotateY(180deg)
  }
  .ant-table{
    background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).whiteBackground};
  }
  &.table-order,
  &.table-seller,
  &.table-data-view{
    .ant-table-selection{
      .ant-checkbox-indeterminate{
        .ant-checkbox-inner{
          background: ${(props: ExecutionContext) => props.theme.colors.primaryColor};
          border-color: ${(props: ExecutionContext) => props.theme.colors.primaryColor};
          &:after{
            height: 2px;
            background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).whiteBackground};
          }
        }
      }
    }
    .ant-table-container{
      padding-bottom: 25px;
      border-bottom: 1px solid ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorDefault};
    }
    tbody{
      tr{
        &:hover{
          td{
            background: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).mainBackground};
          }
        }
        td{
          .product-id{
            max-width: 60px;
            text-align: left;
          }
        }
      }
    }
    .ant-pagination{
      margin-top: 25px !important;
    }
  }
  &.table-data-view{
    .ant-table-container{
      padding-bottom: 15px;
    }
    table{
      thead{
        th{
          padding: 16px 25px;
        }
      }
      tbody{
        td{
          padding: 16px 25px;
          .record-img{
            img{
              max-width: 38px;
              width: 38px;
              height: 38px;
              border-radius: 50%;
              margin-left: 12px;
            }
          }
          .record-location{
            display: block;
            font-size: 12px;
            font-weight: 400;
            color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).lightText};
          }
          .status{
            font-weight: 500;
            text-transform: capitalize;
            &.active{
              color: ${(props: ExecutionContext) => props.theme.colors.successColor};
              background: ${(props: ExecutionContext) => props.theme.colors.successColor}10;
            }
            &.deactivated{
              color: ${(props: ExecutionContext) => props.theme.colors.warningColor};
              background: ${(props: ExecutionContext) => props.theme.colors.warningColor}10;
            }
            &.blocked{
              color: ${(props: ExecutionContext) => props.theme.colors.dangerColor};
              background: ${(props: ExecutionContext) => props.theme.colors.dangerColor}10;
            }
          }
          .table-actions{
            a{
              svg, i{
                width: 16px;
                color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).extraLightText};
              }
              &.edit{
                &:hover{
                  svg,
                  i{
                    color: ${(props: ExecutionContext) => props.theme.colors.infoColor};
                  }
                }  
              }
              &.delete{
                &:hover{
                  svg,
                  i{
                    color: ${(props: ExecutionContext) => props.theme.colors.dangerColor};
                  }
                }  
              }
            }
          }
        }
      }
    }
  }
  &.table-responsive{
    table{
      tbody{
        tr{
          &:hover{
            td{
              background: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).whiteBackground};
            }
          }
        }
      } 
    }
  }
  table{
    thead{
      tr{
        border-radius: 10px;
        th{
          border-bottom: 0 none;
          &:first-child{
            border-radius: 10px 0 0 10px !important;
          }
          &:last-child{
            border-radius: 0 10px 10px 0 !important;
            text-align: left;
          }
          color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText};
          background: ${(props: ExecutionContext) => props.theme.colors.bgColorLight};
        }
      }
    }
    tbody{
      tr{
        &:hover{
          td{
            background: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).whiteBackground};
          }
        }
        &.ant-table-row-selected{
          &:hover{
            td{
              background: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).whiteBackground};
            }
          }
          td{
            background: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).whiteBackground};
          }
        }
        td{
          border: 0 none;
          font-weight: 500;
          color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).darkText};
          &:first-child{
            border-radius: 10px 0 0 10px !important;
          }
          &:last-child{
            border-radius: 0 10px 10px 0 !important;
          }
          span{
            display: block;
          }
          .order-id{
            min-width: 128px;
          }
          .customer-name{
            min-width: 174px;
          }
          .status{
            min-width: 175px;
          }
          .ordered-amount{
            min-width: 175px;
          }
          .ordered-date{
            min-width: 165px;
          }
          .table-actions{
            min-width: 60px;
          }
        }
      }
    }
    .table-actions{
      text-align: left;
      min-width: 150px !important;
      button{
        height: 40px;
        padding: 0 11px;
        background: transparent;
        border: 0 none;
        color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).extraLightText};
        &:hover{
          &.ant-btn-primary{
            color: ${(props: ExecutionContext) => props.theme.colors.primaryColor};
            background: ${(props: ExecutionContext) => props.theme.colors.primaryColor}10;
          }
          &.ant-btn-info{
            color: ${(props: ExecutionContext) => props.theme.colors.infoColor};
            background: ${(props: ExecutionContext) => props.theme.colors.infoColor}10;
          }
          &.ant-btn-danger{
            color: ${(props: ExecutionContext) => props.theme.colors.dangerColor};
            background: ${(props: ExecutionContext) => props.theme.colors.dangerColor}10;
          }
        }
      }
    }
    .seller-info{
      img{
        margin-left: 12px;
      }
    }
    .user-info{
      display: flex;
      align-items: center;
      figure{
        margin: 0 8px 0;
      }
      .user-name{
        margin-bottom: 4px;
        font-weight: 500;
      }
      .user-designation{
        font-size: 12px;
        font-weight: 400;
        color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayLightText};
      }
    }
  }
  .table-actions{
      margin: -8px;
      a{
        display: inline-block;
        margin: 8px;
      }
  }
  .ninjadash-status{
    display: inline-flex;
    align-items: center;
    justify-content: center;
    font-size: 12px;
    text-transform: capitalize;
    font-weight: 500;
    padding: 0 8px;
    min-height: 24px;
    border-radius: 15px;
    &.ninjadash-status-active{
      color: ${(props: ExecutionContext) => props.theme.colors.successColor};
      background-color: ${(props: ExecutionContext) => props.theme.colors.successColor}15;
    }
    &.ninjadash-status-deactiveted{
      color: ${(props: ExecutionContext) => props.theme.colors.warningColor};
      background-color: ${(props: ExecutionContext) => props.theme.colors.warningColor}15;
    }
    &.ninjadash-status-blocked{
      color: ${(props: ExecutionContext) => props.theme.colors.dangerColor};
      background-color: ${(props: ExecutionContext) => props.theme.colors.dangerColor}15;
    }
  }
`;

const DragDropStyle = Styled.div`
  .ant-card-body{
    padding: 15px !important;
  }
  table{
    thead{
      display: none;
    }
    tbody{
      >tr{
        &:not(:last-child){
          td{
             border-bottom: 1px solid ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorSecondary} !important;
          }
         }
         &:hover{
           td{
             background-color: transparent !important;
           }
         }
        >td{
          font-size: 14px;
          &:first-child,
          &:last-child{
            border-radius: 0 !important;
          }
        }
      }
    }
    tr{
      td{
        &.drag-visible{
          svg,
          img{
            width: 16px;
            height: 16px;
          }
          svg,
          i{
            color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).extraLightText};
          }
        }
      }
    }
    .user-info{
      .user-name{
        font-size: 14px;
        margin-left: 8px;
      }
    }
  }
`;

const ImportStyleWrap = Styled.div`
  @media only screen and (max-width: 575px){
    margin-top: 10px;
  }
  .ant-upload.ant-upload-drag{
    background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).whiteBackground};
    min-height: 280px;
    display: flex;
    align-items: center;
    border-color: #C6D0DC;
    border-radius: 10px;
    .ant-upload-drag-icon{
      svg,
      i{
        color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).extraLightText};
      }
    }
  }
  .ninjadash_import-inner{
    .ant-upload-text{
      font-size: 20px;
      font-weight: 500;
    }
    .ant-upload-hint{
      margin-left: 4px;
      span{
        color: ${(props: ExecutionContext) => props.theme.colors.primaryColor};
      }
    }
    .ant-upload-list{
      .ant-upload-list-item{
        background-color: #fff;
        border-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorDefault};
      }
      .ant-upload-list-item-card-actions.picture{
        top: 18px;
        right: 15px;
      }
    }
  }
`;

const ExportStyleWrap = Styled.div`
  .ninjadash_export-box{
    display: flex;
    justify-content: space-between;
    margin-bottom: 30px;
    @media only screen and (max-width: 575px){
      flex-flow: column;
    }
    .btn-export{
      height: 44px; 
      @media only screen and (max-width: 575px){
        margin-bottom: 20px;
      }
    }
    .ant-select{
      width: auto !important;
      .ant-select-selector{
        padding: 0 20px;
        .ant-select-selection-search-input{
          border-radius: 20px;
          background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).inputBg};
          border-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorDefault};
          input{
            background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).inputBg};
          }
        }
      }
    }
  }
  .ninjadash_export-file-table{
    .ant-table{
      border-radius: 10px 10px 0 0;
      background-color: transparent;
    }
    .ant-table-content{
      .ant-table-thead{
        border: 1px solid ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorDefault};
        background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).mainBackgroundLight};
        border-radius: 10px;
        tr{
          th{
            background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).mainBackgroundLight};
            border: 0 none;
            color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText};
            &:first-child{
              border-radius: 10px 0 0 10px;
            }
            &:last-child{
              border-radius: 0 10px 10px 0;
            }
          }
        }
      }
      .ant-table-tbody{
        tr{
          &:hover{
            box-shadow: 0 15px 50px #9299B820;
            td{
              background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).whiteBackground} !important;
            }
          }
          td{
              border: 0 none;
              padding: 22px 25px;
              color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText};
              background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).whiteBackground};
            }
          }
        }
      }
    }
  }
  .ninjadash-button-grp{
    button{
      min-height: 44px;
    }
  }
`;

const CollapseStyleWrap = Styled.div`
  .ant-collapse{
    background-color: transparent !important;
    > .ant-collapse-item{
      margin-bottom: 5px;
      border-radius: 5px;
      background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).whiteBackground};
      border: 1px solid ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).lightBorder} !important;
      box-shadow: 0px 15px 40px rgba(173,181,217,.15);
      > .ant-collapse-header{
        font-size: 15px;
        font-weight: 500;
        color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).darkText};
        padding: 18.5px 25px 18.5px 25px;
        border-bottom: 1px solid ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).lightBorder};
        .ant-collapse-arrow{
          margin-right: 8px;
          vertical-align: -2px;
        }
        svg{
          color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).extraLightText};
        }
      }
      .ant-collapse-extra{
        span{
          font-size: 14px;
          font-weight: 400;
          color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText};
        }
      }
      > .ant-collapse-content{
        .ant-collapse-content-box{
          padding: 20px 25px 12px;
        }
        p{
          font-size: 16px;
          line-height: 1.7;
          color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText};
        }
      }
    }
  }
`;

const TopToolBox = Styled.div`
  margin-bottom: 30px;
  .ninjadash-showcase-top{
    .ant-select {
      .ant-select-selection-search {
        .ant-select-selection-search-input{
          min-width: auto;
        }
      }
    }
    .ninjadash-showcase-top__text{
      p{
        font-size: 14px;
        margin-left: 0;
        color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText};
      }
    }
    .ninjadash-showcase-top__action{
      display: flex;
      align-items: center;
      .ninjadash-showcase-top__action--filter{
          margin-right: 50px;
          .toolbox-menu-title{
            display: inline-block;
            margin-right: 20px;
            font-size: 14px;
            color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText};
          }
          .ant-select-single:not(.ant-select-customize-input) .ant-select-selector{
            padding: 0 10px;
            background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).inputBg};
            border-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorDefault} !important;
            .ant-select-selection-item{
              font-size: 14px;
              color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).menuActive};
            }
          }
          .ant-select{
            .ant-select-arrow{
              color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText};
            }
          }
      }
      .ninjadash-showcase-top__action--viewmode{
        display: flex;
        align-items: center;
        a{
          display: flex;
          align-items: center;
          justify-content: center;
          width: 40px;
          height: 40px;
          border-radius: 50%;
          background-color: transparent;
          cursor: pointer;
          box-shadow: 0 0 0 rgba(173,181,217,.10);
          svg{
            width: 16px;
            height: 16px;
            color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).lightText};
          }
          &.active{
            background-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).whiteBackground};
            box-shadow: 0 5px 10px rgba(173,181,217,.10);
            svg{
              color: ${(props: ExecutionContext) => props.theme.colors.primaryColor};
            }
          }
        }
      }
    }
  }
  
  .table-toolbox-menu{
    margin: -10px;
    color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText};
    @media only screen and (max-width: 1599px){
      text-align: left;
    }
    @media only screen and (max-width: 991px){
      margin-top: 20px;
    }
    .ant-radio-button-wrapper{
      height: 40px;
      line-height: 40px;
      padding: 0 12.5px;
      &.active{
        span{
          color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).menuActive};
        }
      }
    }
    @media only screen and (max-width: 991px){
      text-align: center;
    }
    .toolbox-menu-title,
    .ant-radio-group-outline{
      margin: 10px;
    }
  }
  .ant-select{
    @media only screen and (max-width: 1599px){
      margin-bottom: 20px;
    }
    @media only screen and (max-width: 767px){
      max-width: 350px;
      margin: 0 auto 20px;
    }
    .ant-select-selection-search{
      @media only screen and (max-width: 991px){
        width: 100% !important;
      }
      .ant-select-selection-search-input{
        min-width: 350px;
        @media only screen and (max-width: 1792px){
          min-width: 230px;
        }
      }
    }
  }
  .search-result{
    margin: 0 25px 0 0;
    color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText};
    @media only screen and (max-width: 1599px){
      text-align: left;
      margin-bottom: 15px;
    }
    @media only screen and (max-width: 991px){
      text-align: center;
      margin-bottom: 18px;
    }
    @media only screen and (max-width: 991px){
      margin-left: 0px;
    }
  }
  .ant-select-selector{
    height: 46px !important;
    .ant-select-selection-search-input{
      box-shadow: 0 5px 20px ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).lightText}3;
      border-radius: 23px;
      border: 0 none;
      input{
        height: 46px !important;
      }
    }
  }

  .ant-radio-group-outline{
    padding: 0 10px;
    border-radius: 5px;
    background: #fff;
    border: 1px solid ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorDefault};
    @media only screen and (max-width: 1792px){
      padding: 0 5px;
    }
    @media only screen and (max-width: 991px){
      padding: 0;
    }
  }
  .ant-radio-button-wrapper{
    height: 40px;
    line-height: 42px;
    padding: 0 12px;
    border-color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).borderColorSecondary};
    border: 0 none !important;
    @media only screen and (max-width: 1792px){
      padding: 0 7.5px;
    }
    @media only screen and (max-width: 1599px){
      padding: 0 12.5px;
    }
    &.ant-radio-button-wrapper-checked{
      &:focus-within{
        box-shadow: 0 0;
      }
    }
    &:not(:first-child){
      &:before{
        display: none;
      }
    }
    &:not(:last-child){
      &:after{
        position: absolute;
        top: 50%;
        transform: translateY(-50%);
        left: 0px;
        display: block;
        box-sizing: content-box;
        width: 1px;
        height: 50%;
        padding: 1px 0;
        background-color: #F1F2F6;
        transition: background-color 0.3s;
        content: '';
        z-index: 1;
        @media only screen and (max-width: 479px){
          display: none;
        }
      }
    }
    span{
      color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).lightText};
      @media only screen and (max-width: 1792px){
        font-size: 13px;
      }
    }
    &.ant-radio-button-wrapper-checked{
      span{
        color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).menuActive};
      }
    }
  }

  // Product Toolbox Styles
  .product-list-action{
    @media only screen and (max-width: 991px){
      flex-flow: column;
      justify-content: center;
    }
  }
  .product-list-action__tab{
    margin: -10px;
    color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).grayText};;
    
    @media only screen and (max-width: 767px){
      margin-bottom: 15px;
      text-align: center;
    }
    @media only screen and (max-width: 991px) and (min-width: 768px){
      margin: -10px -10px 0;
    }
    @media only screen and (max-width: 575px){
      margin: -6px -6px 0;
    }
    @media only screen and (max-width: 344px){
      .ant-radio-group-outline{
        margin-top: 8px;
        margin-left: 0;
      }
    }
    .toolbox-menu-title,
    .ant-radio-group{
      margin: 10px;
      @media only screen and (max-width: 575px){
        margin: 6px
      }
    }
  }

  .product-list-action__viewmode{
    display: flex;
    align-items: center;
    a{
      display: inline-flex;
      align-items: center;
      justify-content: center;
      width: 40px;
      height: 40px;
      border-radius: 50%;
      color: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).lightText};
      &.active{
        background-color: #fff;
        box-shadow: 0 5px 20px ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).lightText}10;
        color: ${(props: ExecutionContext) => props.theme.colors.primaryColor};
      }
    }
  }

  .table-search-box{
    @media only screen and (max-width: 991px){
      max-width: 600px;
      margin: 0 auto;
    }
    .ant-select{
      margin-bottom: 0;
      .ant-select-selector{
        .ant-select-selection-search{
          width: 100% !important;
          .ant-select-selection-search-input {
            border-radius: 20px;
            border: 0 none;
            background: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).mainBackgroundLight};
            height: 40px;
            input{
              background: ${(props: ExecutionContext) => getThemeModeColors(props.theme.config.mode, props.theme.colors).mainBackgroundLight};
              height: 40px !important;
            }
          }
        }
      }
    }
  }
  .table-toolbox-actions{
    text-align: left;
    display: flex;
    justify-content: flex-end;
    align-items: center;
    @media only screen and (max-width: 1599px){
      margin-top: 20px;
      justify-content: center !important;
      text-align: center !important;
    }
    button{
      padding: 0px 13.4px;
      height: 38px;
      font-size: 13px;
      font-weight: 500;
      border-radius: 6px;
      svg,
      i{
        color: #fff;
      }
      &{
        +button{
          margin-left: 8px;
        }
      }
    }
  }
`;

export {
  Main,
  ErrorWrapper,
  BorderLessHeading,
  TableDefaultStyle,
  ButtonsGroupWrapper,
  BlockButtonsWrapper,
  ButtonSizeWrapper,
  BtnWithIcon,
  AlertList,
  AutoCompleteWrapper,
  CalendarWrapper,
  DatePickerWrapper,
  NotificationListWrapper,
  TagInput,
  PageHeaderWrapper,
  MessageStyleWrapper,
  BasicFormWrapper,
  CardToolbox,
  FormGroupWrapper,
  DragDropStyle,
  BannerCardStyleWrap,
  FileCardWrapper,
  TableWrapper,
  ImportStyleWrap,
  ExportStyleWrap,
  ChartPointHorizontal,
  CollapseStyleWrap,
  TopToolBox,
};
