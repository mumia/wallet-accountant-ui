...
export default function NewTagCategory({ onClose, visible }: Control) {
  ...
  const handleOk = async () => {
    messageApi.open({
      type: "loading",
      content: "Adding new tag category..",
      duration: 0
    });
    ...
    try {
      const tagCategory = form.getFieldsValue();
      const success = await api.newTagCategory(tagCategory);

      if (success) {
        messageApi.destroy();
        messageApi.open({
          type: "success",
          content: "Tag category added"
        });
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Form.Item name="categoryNotes" label="Category notes" initialValue={``}>
          <Input.TextArea rows={2} placeholder="Category notes" />
        </Form.Item>
